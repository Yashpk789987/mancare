...
import { Container, Content, Title, Text, Button, Icon, CheckBox, ListItem, Body, Header, Left, Right } from 'native-base'
...
class OldReport extends React.Component {
  ...
  oldReportUpdate() {
    if(this.state.old_report && this.state.request_to_admin) {
      this.props.ToggeleReportStatus('old_report', this.state.date,this.state.comment);
      ...
      return (
        <Container >
          <Header style={{ backgroundColor: 'blue'}}>
            ...
            <Body><Title style={{ marginLeft: 5}}>Report Status</Title></Body>
            <Right />
          </Header>
          <Content contentContainerStyle={{ flex: 1, justifyContent: 'center', alignContent: 'center', padding: 50 }}>
            <Text style={{ fontSize: 20 }}>Report Status: Old Report</Text>
            <Button style={{ backgroundColor: 'red', width: 200, justifyContent: "center", margin: 20 }} onPress={() => this.props.ToggeleReportStatus()}>
    ...
    return(
      <Container >
        <Header>
          ...
          <Body>
            <Text style={{ fontSize: 20, color: 'white'}}> Report Information</Text>
          </Body>

        </Header>
        <Content contentContainerStyle={{ flex: 1, padding: 30, backgroundColor: 'white' }}>
          <ListItem>
           <CheckBox
             checked={this.state.old_report}
             onPress={() => this.setState({old_report: !this.state.old_report})}
           />
           <Body>
         ...
         <ListItem>
          <CheckBox
            ...
            onPress={() => this.setState({request_to_admin: !this.state.request_to_admin})}
          />
          <Body>
        ...
        <ListItem>
          <Body style={{ flex: 1, justifyContent: "center"}}>
            <Button style={{ width: 200, justifyContent: "center"}} onPress={() => this.oldReportUpdate()}>
